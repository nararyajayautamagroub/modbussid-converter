...
from .template_classifier import TEMPLATES, classify_template
...
def infer_category(path: Path) -> str:
    ...
    for folder, label in platform_roots.items():
        if folder not in parts:
            continue
        root_path = next(
            (parent for parent in path.parents if parent.name.lower() == folder),
            None,
        )
        ...
        return f"{label}/{top}" if top else label
