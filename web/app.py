...
def categories():return {'bussid':['vehicle','map','kodename','sprite-sheet','lights'],'ets2':['vehicle','map'],'roblox':['vehicle','lights','export']}
...
def download(light_type:str,platform:str='roblox'):
    ...
    with TemporaryDirectory() as tmp:path=export_light_package(light_type,Path(tmp)/f'{platform}_{light_type}',platform); data=path.read_bytes()
    saved=out/path.name;saved.write_bytes(data);return FileResponse(saved,filename=saved.name,media_type='application/zip')
